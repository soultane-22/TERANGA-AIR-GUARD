    ...
    <div className={`flex h-screen font-sans overflow-hidden transition-colors duration-300 ${isDarkMode ? 'dark bg-slate-950 text-white' : 'bg-[#F8FAFC] text-slate-800'}`}>
      ...
      <aside className={`${isSidebarOpen ? 'w-72' : 'w-24'} bg-[#0F172A] border-r border-slate-800 text-white transition-all duration-300 flex flex-col z-40 flex-shrink-0`}>
        ...
        <div className="mt-auto p-4 border-t border-slate-800/50 space-y-2">
          <button onClick={() => window.open('http://13.61.185.172:8000/api/export-excel', '_blank')} className="w-full flex items-center p-3.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 rounded-xl transition-all group">
...
function PredictionsView() {
    ...
    useEffect(() => {
        axios.get('http://13.61.185.172:8000/api/dashboard-stats').then(res => {
            setStats(res.data);
...
function MapView() {
    ...
    useEffect(() => {
        const fetchMapData = async () => {
            try {
                const res = await axios.get('http://13.61.185.172:8000/api/map-sensors');
                setSensors(res.data);
...
function AlertsList() {
    ...
    useEffect(() => {
        axios.get('http://13.61.185.172:8000/api/dashboard-stats').then(res => setStats(res.data)).catch(console.error);
    }, []);
...
function HistoryView() {
    ...
    useEffect(() => { axios.get('http://13.61.185.172:8000/api/history').then(res => setHistoryData(res.data)).catch(console.error); }, []);
    ...
    const handleExportClick = () => {
        ...
        window.open(`http://13.61.185.172:8000/api/export-excel?${params.toString()}`, '_blank');
    };
...
function DeviceManagementView() {
    const [stats, setStats] = useState(null);
    useEffect(() => { axios.get('http://13.61.185.172:8000/api/dashboard-stats').then(res => setStats(res.data)).catch(console.error); }, []);
...
function SystemHealthView() {
    ...
    useEffect(() => {
        const fetchHealth = () => {
            axios.get('http://13.61.185.172:8000/api/system-health')
